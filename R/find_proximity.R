...
##'
##' @param target numeric; number of the target (reference) station
##' @param mindist numeric; minimum distance from the target station (defaults to 0)
##' @param maxdist numeric; maximum distance from the target station (defaults to 100)
##' @param sort Boolean; if set to TRUE (default), will sort the resultant table by distance from `target`
##'
...
find_proximity <- function(target, mindist = 0, maxdist = 100, sort = TRUE) {
  ...
  tempdata$LongLat <- rep(NA, nrow(tempdata))
  tempdata$Dist <- rep(NA, nrow(tempdata))
  
  for (i in 1:nrow(tempdata)) {
    tempdata$Dist[i] <- (geosphere::distGeo(p1, c(tempdata$LongitudeDD[i], tempdata$LatitudeDD[i]))/10000)
  }
